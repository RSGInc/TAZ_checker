...
def toArray(fiter, varlist):
    '''
    A function that, when given a list of fields, returns a tuple of values with
    a 0 for the value if it doesn't exist in the input data file.
    '''
...
def addResult(fiter, fields, value_array, array_fields):
    [updateRow(outrow, field, row[arr_field]) for outrow, row in zip(fiter.data_parsed[
        'features'], value_array) for field, arr_field in zip(fields, array_fields)]
